  ...
  const { data: allCategories = [], isLoading: loadingCategories } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("categories")
        .select(
          "id, name, slug, sort_order, parent_id, cover_image_url, cover_thumbnail_url, cover_medium_url, cover_full_url",
        )
        .eq("is_hidden", false)
        .is("deleted_at", null)
        .order("sort_order");
  ...
  const { data: categoryCoverUrls = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const root of categories) {
        if (root.cover_image_url || root.cover_medium_url) {
          covers[root.id] = resolveProductImageUrl(
            {
              image_url: root.cover_image_url ?? root.cover_medium_url!,
              thumbnail_url: root.cover_thumbnail_url ?? null,
              medium_url: root.cover_medium_url ?? null,
              full_url: root.cover_full_url ?? null,
            },
            "medium",
          );
          continue;
