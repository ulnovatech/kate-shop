...
describe("ADMIN_ROUTE_CATALOG", () => {
  it("lists 23 staff routes for C14 parity", () => {
    expect(ADMIN_ROUTE_CATALOG).toHaveLength(23);
  });
  ...
  it("covers all blueprint areas", () => {
    expect(adminRoutesInArea("auth")).toHaveLength(5);
    ...
    expect(adminRoutesInArea("team")).toHaveLength(4);
  });

  it("marks public auth routes", () => {
    expect(ADMIN_PUBLIC_ROUTE_IDS).toEqual(["login", "join", "signup", "setup", "accept-invite"]);
    expect(ADMIN_PROTECTED_ROUTE_CATALOG).toHaveLength(18);
  });
});
