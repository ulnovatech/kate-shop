...
function Checkout() {
  ...
  const handleWizardContinue = async () => {
    ...
    if (wizardStep === 3) {
      ...
      if (!ok) return;
      if (!delivery.valid) {
        toast.error("Select a valid delivery area");
        return;
      }
      setWizardStep(nextCheckoutWizardStep(wizardStep));
    }
  ...
  const onSubmit = async (values: CheckoutFormData) => {
    if (!online) {
      toast.error("You are offline. Connect to the internet to place an order.");
      return;
    }
    ...
    try {
      ...
      if (stockCheck.backorderRequired) {
        toast.message("Some items are low on stock", {
          description:
